...
export default function CClubs() {
  ...
  const [modalClub, setModalClub] = useState<typeof CulturalClubs[0] | null>(null);
  ...
  const handleClubClick = (index: number) => {
    if (isMobile) {
      setModalClub(CulturalClubs[index]);
      setModalOpen(true);
    } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 py-8 md:py-16 px-4">
      ...
      <div className="text-center mb-8 md:mb-16">
        <div className="inline-block">
          ...
          <p className="text-gray-400 text-base md:text-lg">Discover the creative spirits of our campus</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {CulturalClubs.map((club, index) => (
              <div
                key={index}
                className={`${
                  CulturalClubs.length % 2 !== 0 && index === CulturalClubs.length - 1
      ...
      <div className="text-center mt-8 md:mt-12">
        ...
        <p className="text-gray-400 text-sm">
          {isMobile ? 'Tap cards to view details' : 'Hover to preview. Click to expand. Click again to collapse'}
        </p>
      </div>
